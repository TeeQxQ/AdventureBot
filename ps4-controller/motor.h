 ...
 */
 ...
 class Motor
 {
  public:
    ...
    typedef struct Config
    {
      ...
      Config(const int pin_direction_in1,
             const int pin_direction_in2,
             const int pin_pwm,
             const int pwm_channel): pin_direction_in1(pin_direction_in1), 
    ...
    Motor(const int pin_direction_in1,
          ...
          const int pin_pwm,
          const int pwm_channel,
          const Motor::Direction defaultDirection) : config(pin_direction_in1, pin_direction_in2, pin_pwm, pwm_channel),
          ...
          
    {
      ...
      ledcSetup(this->config.pwm_channel, this->freq, this->resolution);
      ledcAttachPin(this->config.pin_pwm, this->config.pwm_channel);
    ...
    void stop()
    {
      ...
      ledcWrite(this->config.pwm_channel, this->MIN_PWM_SPEED);
    }
    ...
    void changeSpeed()
    {
      ...
      ledcWrite(this->config.pwm_channel, this->convertSpeedToPWM());
    }
  ...
  private:
    ...
    const int MAX_PWM_SPEED = 1023; //PWMRANGE;
